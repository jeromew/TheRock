...
def main():
    parser = argparse.ArgumentParser(description="Run rocfft tests")
    parser.add_argument(
        "--multi_gpu",
        action="store_true",
    ...
    if args.multi_gpu:
        if test_type == "quick":
            logging.info("Skipping rocfft multi-GPU tests: quick test mode")
            return 0
        ...
        if gpu_count < 2:
            logging.warning("Skipping rocfft multi-GPU tests: <2 GPUs visible")
            return 0
