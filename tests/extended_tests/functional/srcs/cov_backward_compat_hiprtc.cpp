...
static int detect_cov(const char *data, size_t size) {
  for (size_t i = 0; i + 16 <= size; ++i) {
    if (data[i] != '\x7f' || data[i + 1] != 'E' || data[i + 2] != 'L' ||
        data[i + 3] != 'F')
      continue;
...
static void emit_json(const char *variant, int requested_cov, int detected_cov,
...
static bool rtc_compile(bool use_default, int cov_version,
  ...
  hiprtcProgram prog;
  if (hiprtcCreateProgram(&prog, kernel_src, "vecadd.hip", 0, nullptr,
  ...
  char flag[64] = {};
  if (!use_default) {
    std::snprintf(flag, sizeof(flag), "-mcode-object-version=%d", cov_version);
    opts.push_back(flag);
  }

  hiprtcResult comp =
      hiprtcCompileProgram(prog, static_cast<int>(opts.size()),
...
static bool load_and_verify(const std::vector<char> &code) {
...

  {
    ...
    if (hipModuleLaunchKernel(func, blocks, 1, 1, THREADS_PER_BLOCK, 1, 1, 0,
                              nullptr, nullptr, config) != hipSuccess)
      goto cleanup;
  }
  ...
  for (int offset = 1; offset <= 2; ++offset) {
    ...
    if (target < 2) {
      ...
      std::snprintf(msg, sizeof(msg),
                    "version %d below minimum (derived from n=%d)", target, n);
    ...
    if (detected != target) {
      char msg[128];
      std::snprintf(msg, sizeof(msg), "expected COV %d but detected %d", target,
                    detected);
